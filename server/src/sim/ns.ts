...
//
// The namespace rides inside the uid (WT-SIM-<ns>-E-6): each stack ingests
// only its own namespace (units.ts drops the rest), while real EUD uids —
// which never carry the WT-SIM- prefix — always pass. Default is the server
// port: distinct per parallel stack by construction (two stacks can't bind
// one port) and stable across restarts. Pin it with WATCHTOWER_SIM_NS when
// port-derived isn't wanted.
// ---------------------------------------------------------------------------
...
/**
 * A parallel dev stack's simulated unit: same TAK server, same WT-SIM-
 * family, different namespace. Its fleet belongs to that stack's incident,
 * not ours.
 */
export function isForeignSimUid(uid: string): boolean {
  return uid.startsWith('WT-SIM-') && !uid.startsWith(SIM_UID_PREFIX)
}
